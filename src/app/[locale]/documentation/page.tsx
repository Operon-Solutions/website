...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${dict.documentation.title} | Operon`,
    description: dict.documentation.subtitle,
  };
}
      ...
      <section className="pb-24">
        <div className="max-w-[720px] mx-auto px-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {d.categories.map((cat: { title: string; description: string; slug: string }) => (
              <Link
                key={cat.slug}
                href={`/${locale}/docs/${cat.slug}`}
                className="group border border-fg/[0.06] rounded-xl p-5 hover:border-accent/20 transition-colors"
              >
                <h3 className="text-[15px] font-medium mb-2 group-hover:text-accent transition-colors">{cat.title}</h3>
                <p className="text-[13px] text-fg/30 leading-[1.6]">{cat.description}</p>
                <div className="flex items-center gap-1.5 mt-4 text-[12px] text-accent/50 group-hover:text-accent transition-colors">
