...
我們的目標是**流程圖面數位化**：從 P&ID 或 PFD 的圖面中提取其拓撲結構，並以圖譜的格式儲存為結構化、機器可讀的數據。
...
上傳 PDF、TIFF、DWG、PNG 或掃描紙本格式的 P&ID 圖面。前處理流程負責糾偏、去噪、多頁分割與解析度標準化。
...
![Operon P&ID 識別示範 — 偵測到的元件、標籤號與信心程度](/blog/plant-pid-ai.webp)
...
我們系統的核心是一個為物件偵測任務訓練的深度卷積神經網路 — 它能在一次處理中同時找出圖面上所有元件的位置和類型，而不僅僅是辨認「這張圖是什麼」。

神經網路從圖面中自動學習層次化特徵 — 從基礎的線條和形狀，逐步組合成可辨識的工程符號，如閥門、儀器和設備。

我們的偵測系統能在多個尺度上同時搜尋 — 這對 P&ID 至關重要，因為同一張圖上可能同時出現小型儀表標籤和大型容器。模型為每個偵測結果輸出信心程度，讓我們可以過濾結果，並將不確定的案例標記供人工審查。
...
P&ID 圖面有幾個獨特挑戰，是一般影像辨識技術無法直接處理的：

- **密集、重疊的符號** — 與一般照片不同，P&ID 中的符號緊密排列且管線交錯。模型必須準確區分重疊的元素，而不將一個符號誤認為另一個。
- **小型文字與標籤** — 如「FIC-13310」這樣的標籤號是關鍵資訊，但在圖面上只佔極少像素。我們使用專門針對工程標註風格調優的獨立文字偵測與 OCR 流程。
- **符號出現頻率差異大** — 某些符號（如管線）在每張圖上出現數千次，而其他符號（如洩壓閥）可能只出現一兩次。我們在訓練中採用特殊的權重策略，確保模型不會忽略少見但重要的元件。
- **管線連接性** — 偵測符號只是問題的一半。理解哪些符號連接到哪些符號，需要追蹤圖面中的管線和信號線。我們透過獨立的路徑探索演算法來處理，根據偵測到的符號位置和管線走向進行分析。

### 持續改進

模型會隨時間不斷進步。隨著我們為更多客戶、跨更多產業處理更多 P&ID，我們持續擴充和精煉訓練數據集。人工審查中修正的偵測結果會回饋到訓練流程 — 這是一個標準的主動學習迴圈，能系統性地解決模型的弱點。
...
**想親自體驗？** [試用 P&ID 識別 Beta 版 →](https://operon-solutions-pid.vercel.app/)
`,
      },
      "pid-agent": {
        title: "CAD / P&ID AI Agent",
...
## CAD / P&ID AI Agent 的功能

Operon 的 CAD / P&ID AI Agent 是一款**專為化工領域打造的 AI**，可從自然語言描述、模擬數據或草圖自動生成工程圖面。
...
CAD / P&ID AI Agent 搭配我們的 P&ID 識別引擎和駐場工程師效果最佳。我們的團隊會根據您的符號庫、標籤慣例和設計標準進行配置。
`,
      },
      "native-plugin": {
        title: "製程模擬軟體原生外掛",
